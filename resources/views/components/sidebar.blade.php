    ...
    <div class="bg-white shadow-lg w-64 flex-shrink-0" x-data="{ sidebarOpen: true }">
        ...
        <nav class="mt-6 px-4">
            <div class="space-y-2">
                ...
                <!-- TTD Settings -->
                @can('admin')
                <a href="{{ route('ttd-settings.index') }}" 
                   class="flex items-center px-4 py-3 text-sm font-medium rounded-lg {{ request()->routeIs('ttd-settings.*') ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
